...
dat4 <- dat3 %>% select(factorNew, ecosystem, factor2) %>%
...
with(datL, table(ecosystem, factorAll))

tbl2 <- ddply(datL, .(ecosystem, factorAll), summarise, 
              freq = length(ecosystem), .drop = FALSE) # frequency, I want %
tbl2

n <- with(datL, table(ecosystem))
str(n)
n <- as.data.frame(n)
n
...
with(datL, table(ecosystem))
sum(with(datL, table(ecosystem)))
# 81 total responses
